...
class TestMoFreezePlaceholderTFFE(unittest.TestCase):
    def test_conversion_fake_pb_model(self):
        ...
        for use_new_frontend, use_legacy_frontend, framework in test_cases:
            with self.subTest(use_new_frontend=use_new_frontend, use_legacy_frontend=use_legacy_frontend, framework=framework):
                with self.assertRaisesRegex(Exception,
                     ...
                     convert_model(input_model, use_new_frontend=use_new_frontend, use_legacy_frontend=use_legacy_frontend, framework=framework)
    ...
    def test_conversion_empty_model(self):
        ...
        for use_new_frontend, use_legacy_frontend, framework, exp_reg_exp in test_cases:
            with self.subTest(use_new_frontend=use_new_frontend, use_legacy_frontend=use_legacy_frontend, framework=framework, exp_reg_exp=exp_reg_exp):
    ...
    def test_conversion_fake_model_with_no_ext(self):
        ...
        for use_new_frontend, use_legacy_frontend, framework, exp_reg_exp in test_cases:
            with self.subTest(use_new_frontend=use_new_frontend, use_legacy_frontend=use_legacy_frontend, framework=framework, exp_reg_exp=exp_reg_exp):
                    with tempfile.NamedTemporaryFile(mode='w') as tmp, self.assertRaisesRegex(Exception, exp_reg_exp):
                        tmp.write("1212234\n12313")
                        convert_model(tmp.name,
                                      use_new_frontend=use_new_frontend,
                                      use_legacy_frontend=use_legacy_frontend,
